...
export default function createContext(i18n, opts = {}) {
	...
	return {
		...
		set locale(value) { localeRef.value = value; },
		[$lookup]: lookup,
	};
...
function lookupChain(messages, locale, key) {
	...
	return findFromChain(locale, locale => get(messages[locale], key));

}

// # findFromChain(locale, cb)
function findFromChain(locale, cb) {
	for (let i = locale.length-1; i >= 0; i--) {
		if (locale.charAt(i) === '-') {
			let partial = locale.slice(0, i);
			let result = cb(partial);
...
function lookupFallbacks(fallbacks, locale) {
	if (Array.isArray(fallbacks)) {
		...
		fb = findFromChain(locale, locale => fallbacks[locale]);
		if (fb) {
			return Array.isArray(fb) ? fb : [fb];
		}
	} else if (fallbacks) {
		return [fallbacks];
	}
