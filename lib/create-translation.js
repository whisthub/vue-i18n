...
export default function createTranslation(i18n, opts = {}) {
	...
	return function translate(key, ctx) {
		...
		if (message === undefined) {
			...
			// Check if a fallback locale is defined, and then lookup the 
			// message again if that's the case.
			const { fallbackLocale } = i18n;
			if (fallbackLocale && fallbackLocale !== locale) {
				if (!i18n.config.silentFallbackWarn) {
					console.warn(
						`Using fallback locale "${fallbackLocale}" for key "${key}"!`,
					);
				}
				message = lookupMessage(i18n, messages, fallbackLocale, key);
			}

			// If the message is still undefined, then return the key as is, but 
			// issue a warning.
			if (message === undefined && !i18n.config.silentTranslationWarn) {
				console.warn(
					`No message "${key}" found for locale "${locale}"!`,
				);
				return key;
			}
