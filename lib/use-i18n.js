...
import { $i18n, $global, $local, $useComponent } from './symbols.js';
...
export function useI18n(opts = {}) {
	...
	if (scope === 'parent') {
		...
		return getParentContext(instance, opts[$useComponent]) || i18n[$global];
	}
...
function getParentContext(instance, useComponent = false) {
	let current = getParentComponentInstance(instance, useComponent);
	while (current) {
		let ctx = current[$local];
		if (ctx) return ctx;
		current = getParentComponentInstance(current, useComponent);
	}
	return null;
}

// See #6. Depending on whether 
